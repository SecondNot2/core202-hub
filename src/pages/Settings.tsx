...
export const SettingsPage: React.FC = () => {
  ...
  const handleAvatarChange = () => {
    // Cycle through some preset avatars
    const avatars = [
    ...
    const currentIdx = avatars.indexOf(user?.avatar);
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-6">
            <div className="relative group">
              <div className="w-20 h-20 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center text-3xl overflow-hidden border-2 border-transparent group-hover:border-violet-500 transition-colors">
                {user?.avatar ? (
                  <img
                    src={user.avatar}
                    alt="Avatar"
              ...
              <button
                className="absolute inset-0 flex items-center justify-center bg-black/40 text-white opacity-0 group-hover:opacity-100 transition-opacity rounded-full font-medium text-xs backdrop-blur-sm"
            ...
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none transition-all"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-100 dark:border-gray-700 pt-4">
            ...
            <InfoRow
              label="Joined"
              value={new Date(
                (user?.metadata?.createdAt as string) || Date.now()
              ).toLocaleDateString()}
            />
          </div>
        </div>
      ...
      <SettingsSection title="Plugins" description="Manage installed plugins">
        <div className="space-y-3">
          {allPlugins.length === 0 ? (
            ...
            allPlugins.map((plugin: RegisteredPlugin) => (
              <div
